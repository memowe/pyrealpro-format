...
class Chord:
    ...
    Attributes:
        ...
        bass:            Optional bass note of a slash chord as a
                         ``(note, accidental)`` pair, e.g.
                         ``(NoteName.C, None)`` for ``D-/C`` or
                         ``(NoteName.B, Accidental.FLAT)`` for ``C/Bb``.
    ...
    bass: tuple[NoteName, Accidental | None] | None = None
...
class MarkerKind(StrEnum):
    ...
    ``SLASH``        ``p``   Intra-beat slash (preceding chord played on beat)
    ``PUSH``         ``,``   Sub-beat separator (compresses chords without advancing empty cells)
    ================ ======= ================================================
    """
...
class Space:
    """
    An empty space in the progression grid.

    Standard chords and spaces each occupy 1 cell on the 16-cell grid.
    These spaces define the rhythm / timing duration of the preceding chord.
    """


# ── Cell union type ───────────────────────────────────────────────────────────

type Cell = Chord | Space | Barline | SectionMark | TimeSignature | Ending | Marker | TextAnnotation
"""
Union of all valid chord-progression cell types.

The eight members cover the full iReal Pro progression language:

- :class:`Chord`          – a chord symbol
- :class:`Space`          – an empty beat / progression cell
